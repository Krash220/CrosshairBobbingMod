...
public class Render {
    ...
    public static boolean isDebugCrosshair() {
        ...
        return mc.options.debugEnabled && !mc.options.hudHidden && !mc.player.hasReducedDebugInfo() && !mc.options.reducedDebugInfo;
    }
    ...
    public static void updateCameraMatrix(MatrixStack mat, float partialTicks) {
        ...
        mat.mat.peek().getPositionMatrix().multiply(mc.gameRenderer.getBasicProjectionMatrix(((GameRendererAccessor) mc.gameRenderer).getFov(mc.gameRenderer.getCamera(), partialTicks, true)));
    }
