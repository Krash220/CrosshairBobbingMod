...
public class Render {
    ...
    public static boolean isDebugCrosshair() {
        ...
        return mc.options.hideGui || mc.options.getCameraType() != CameraType.FIRST_PERSON || mc.options.renderDebug && !mc.player.isReducedDebugInfo() && !mc.options.reducedDebugInfo;
    }
    ...
    public static void updateCameraMatrix(MatrixStack mat, float partialTicks) {
        ...
        mat.mat.last().pose().multiply(mc.gameRenderer.getProjectionMatrix(((GameRendererAccessor) mc.gameRenderer).getFov(mc.gameRenderer.getMainCamera(), partialTicks, true)));
    }
    ...
    public static void distortion(MatrixStack mat, float partialTicks) {
        ...
        float f = Mth.lerp(partialTicks, mc.player.oPortalTime, mc.player.portalTime) * mc.options.screenEffectScale * mc.options.screenEffectScale;
        if (f > 0.0F) {
